...
// Helper functions can be defined at the top
...
export async function onRequest(context) {
    try {
        // Read from the KV namespace bound to this function
        const dbJson = await context.env.CLAN_DB.get("clan_data");

        if (!dbJson) {
            return new Response(JSON.stringify({ error: 'Database not found. Please run the update process first.' }), {
                status: 404,
