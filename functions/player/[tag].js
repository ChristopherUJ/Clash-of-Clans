...
export async function onRequest(context) {
    try {
        ...
        if (!dbJson) {
            return new Response(JSON.stringify({ error: 'Database is empty.' }), { status: 404 });
        }
        ...
        if (!playerData) {
            return new Response(JSON.stringify({ error: 'Player not found in database.' }), { status: 404 });
        }
        ...
        return new Response(JSON.stringify({ error: 'Failed to get player data.' }), { status: 500 });
    }
}
